...
<script>
    document.getElementById('fullname').addEventListener('input', function (e) {
        this.value = this.value.replace(/[^a-zA-Z\s]/g, '').replace(/\s+/g, ' '); // Remove non-alphabetic characters and extra spaces
    });

    document.getElementById('fullname').addEventListener('blur', function (e) {
        const parts = this.value.trim().split(' ');
        if (parts.length < 3) {
            alert('Please enter your first name, middle initial, and last name.');
            this.focus();
            return;
        }
